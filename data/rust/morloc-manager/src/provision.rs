...
pub fn provision_runtime(scope: Scope, requested: &str) -> Result<(PathBuf, String)> {
    ...
    if !stamp.exists() {
        ...
        // MORLOC_RUST_BIN expects these exact filenames; download to them
        // directly (atomically, checksum-verified against the manifest).
        let nexus_dest = dir.join("morloc-nexus");
        let morloc_dest = dir.join("morloc");
        download_asset(&tag, &assets.libmorloc, &dir.join("libmorloc.so"), &manifest.sha256)?;
        download_asset(&tag, &assets.nexus, &nexus_dest, &manifest.sha256)?;
        download_asset(&tag, &assets.morloc, &morloc_dest, &manifest.sha256)?;
        make_executable(&nexus_dest)?;
        make_executable(&morloc_dest)?;

        // rustmorloc is an rlib compiled locally by `morloc init`; ship its
        // source. Wipe any stale rust/ from an interrupted prior extraction so a
        ...
        let _ = std::fs::remove_dir_all(dir.join("rust"));
