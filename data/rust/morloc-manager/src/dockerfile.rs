...
pub fn generate_dockerfile(input: &DockerfileInput) -> String {
    ...
    // + the morloc compiler + rust sources), supplied via the build context. The
    // COPY destination is shared with the run-side PATH (serve::container_path),
    // so it comes from one constant rather than a literal that must match by hand.
    ...
    out.push_str(&format!("ENV MORLOC_RUST_BIN={runtime_bin}\n"));
    ...
    // the immutable runtime prefix (bin/lib/include); `morloc init` installs the
    // trio + shims here in a proper bin/lib layout. At run time this prefix is
