...
pub fn print_nexus_usage(prog_name: &str) -> ! {
    ...
    eprintln!("  -q, --quiet                 Suppress morloc-emitted log lines on stderr");
    eprintln!();
    eprintln!("Run tracking:");
    eprintln!("  --log-dir DIR               Per-run output dir (logs, summary.json, debug dumps)");
    eprintln!("  --summary PATH              Override summary.json location");
    eprintln!();
    eprintln!("Debug trace (requires --debug at compile time):");
    eprintln!("  --debug-cache-depth N       Max arg dumps per dispatch (default 1; 0 = unlimited)");
    eprintln!("  --debug-cache-max BYTES     Per-arg size cap; k/m/g suffix (default 0 = unlimited)");
    eprintln!("  --debug-recursion-cap N     Max trace frames per midx (default 3; 0 = unlimited)");
    eprintln!();
    ...
    eprintln!("  --eval-timeout SECS         CPU budget for /eval and /typecheck (default: 30)");
    eprintln!();
...
pub fn print_command_help_single(prog_name: &str, cmd: &Command) -> ! {
    ...
    eprintln!("  --quiet                     Suppress morloc-emitted log lines on stderr");
    eprintln!("\nRun tracking:");
    eprintln!("  --log-dir DIR               Per-run output dir (logs, summary.json, debug dumps)");
    eprintln!("  --summary PATH              Override summary.json location");
    eprintln!("\nDebug trace (requires --debug at compile time):");
    eprintln!("  --debug-cache-depth N       Max arg dumps per dispatch (default 1; 0 = unlimited)");
    eprintln!("  --debug-cache-max BYTES     Per-arg size cap; k/m/g suffix (default 0 = unlimited)");
    eprintln!("  --debug-recursion-cap N     Max trace frames per midx (default 3; 0 = unlimited)");
    eprintln!("\nDaemon mode:");
