...
static int recur_env_push(const Schema* schema) {
    ...
    if (recur_env_depth > 0 && recur_env_stack[recur_env_depth - 1].schema == schema) return 0;
...
typedef struct {
    ...
    r_frame_t cur;
    int direct;
    // The last schema asked about by r_flat and its answer: a loop over an
    // array asks about the same element schema once per element.
...
static int r_flat(r_walk_t* w, const Schema* schema) {
    if (w->direct) return 1;
    if (schema != w->flat_schema) {
        w->flat_schema = schema;
        w->flat_answer = !r_schema_has_recur(schema);
    }
