...
spreadAttr :: (Ord a, Ord b) => [(a, Maybe a, Maybe b)] -> Map.Map a b
spreadAttr xs =
  asHash [(Set.toList sx, Set.toList sy) | (sx, sy) <- shareAttr xs]
  where
    asHash :: Ord a => [([a], [b])] -> Map.Map a b
    asHash xys =
      (Map.fromList . concat)
...
shareAttr ::
     (Ord a, Ord b) => [(a, Maybe a, Maybe b)] -> [(Set.Set a, Set.Set b)]
shareAttr xs = foldl f [] xs
  where
    f :: (Ord a, Ord b)
      => [(Set.Set a, Set.Set b)]
      -> (a, Maybe a, Maybe b)
      -> [(Set.Set a, Set.Set b)]
    f ((s, c):ss) (x, y, z) =
      if Set.member x s || maybeMember y s
        then (maybeInsert y (Set.insert x s), maybeInsert z c) : ss
        else (s, c) : (f ss (x, y, z))
