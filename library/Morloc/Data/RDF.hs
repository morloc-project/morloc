...
prefixMap =
  DR.PrefixMappings $
  DMS.fromList
    [("mlc", mlcPre), ("mid", midPre), ("rdf", rdfPre), ("xsd", xsdPre)]
...
instance MorlocNodeLike Bool where
  ...
  fromRdfNode (DR.LNode (DR.TypedL x t))
    ...
    | t == (xsdPre <> "boolean") =
      error ("Expected RDF boolean to be true/false, found: " ++ show x)
    | otherwise =
      error ("Expected boolean node, found: " ++ show x ++ "^^" ++ show t)
...
instance MorlocNodeLike GraphObject where
  ...
  fromRdfNode (DR.BNodeGen x) =
    error ("illegal RDF node: " ++ show (DR.BNodeGen x))
  fromRdfNode n
...
instance G.Pretty DR.Node where
  ...
  pretty (DR.LNode (DR.PlainLL lit lang)) =
    G.textEsc' lit <> "@" <> G.pretty lang
  pretty (DR.LNode (DR.TypedL lit dtype)) =
    G.textEsc' lit <> "^^" <> G.angles (G.pretty dtype)
...
instance RdfLike RDF where
  writeTurtle p x = do
    ...
    DR.hWriteRdf serializer handle x <* SIO.hClose handle
  asTriples = DR.triplesOf

instance SparqlDatabaseLike RDF
  ...
  sparqlSelect t q x
   ...
   = do
    ...
    let cmd =
          "arq --data=" <>
          turtlePath <> " --query=" <> sparqlPath <> " --results=TSV"
...
adoptAs ::
     (MorlocNodeLike s, MorlocNodeLike p) => s -> p -> [TopRDF] -> [DR.Triple]
...
getImports rdf = up rdf PType OImport >>= down rdf OName >>= valueOf
