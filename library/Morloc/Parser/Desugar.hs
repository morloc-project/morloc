...
module Morloc.Parser.Desugar (desugar, parsub) where
...
desugarTypeSet :: Map.Map TVar (Type, [TVar]) -> TypeSet -> MorlocMonad TypeSet
...
desugarType h t0@(ArrT v ts) = case Map.lookup v h of
  (Just (t, vs)) ->
    ...
    then MM.throwError . BadTypeAlias
      $  "Type alias " <> MD.render (MD.pretty v) <> " expected " <> MT.show' (length vs)
      <> " parameters but found " <> MT.show' (length ts)
    else return $ foldr parsub t (zip vs ts)
  Nothing -> return t0
...
parsub (v, t2) t1@(VarT v0)
  | v0 == v = t2
  | otherwise = t1
parsub _ (ExistT _ _ _) = error "What the hell is an existential doing down here?"
