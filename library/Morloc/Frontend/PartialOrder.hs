...
substitute v r t = sub t
  where
    ...
    sub (NamU namType v' rs) = NamU namType v' [(x, sub t') | (x, t') <- rs]
...
free (NamU _ _ rs) = Set.unions [free t | (_, t) <- rs]
...
findFirst v (NamU _ _ es1) (NamU _ _ es2)
  = listToMaybe . catMaybes $ zipWith (findFirst v) ts1 ts2
    where
      ts1 = map snd es1
      ts2 = catMaybes $ map (\(k,_) -> lookup k es2) es1
