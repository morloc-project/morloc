...
instance SparqlDatabaseLike SparqlEndPoint
...
uploadTriples ep xs = updateQueryRaw ep (G.render' . makeSparql $ xs)
  where


makeSparql :: [MR.Triple] -> MDoc
makeSparql xs = "INSERT DATA" <> G.line <> G.braces (G.indent 4 $ G.pretty xs)
