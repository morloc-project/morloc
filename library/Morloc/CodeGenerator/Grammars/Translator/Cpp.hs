...
showType recmap (AppP (PV _ _ v) ts) = pretty $ expandMacro v (map (render . showType recmap) ts)
showType _ (RecP _ _) = undefined -- FIXME record names
...
cleanRecord m tm = case typeOfTypeM tm of
  ...
  where
    ...
    toRecord (AppP _ ts) = conmap toRecord ts
    toRecord (RecP _ _) = undefined -- FIXME: record names
...
generateSourcedSerializers es0
  ...
  where
    ...
    makeSerial = undefined -- FIXME - record names
