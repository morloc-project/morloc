...
instance {-# OVERLAPPABLE #-} HasTypeF e => HasCppType e where
  cppTypeOf = f . typeFof where
    ...
    f t@(NamF _ (FV gc (TV "struct")) _ rs) = do
      recmap <- CMS.gets translatorRecmap
      -- handle autogenerated structs
      case lookup (FV gc (TV "struct"), map fst rs) recmap of
        (Just rec) -> do
...
collectRecords :: SerialManifold -> [(FVar, GIndex, [(Key, TypeF)])]
collectRecords e0@(SerialManifold i0 _ _ _)
  ...
  where
    ...
    seekRecs :: Int -> TypeF -> [(FVar, GIndex, [(Key, TypeF)])]
    seekRecs m (NamF _ v@(FV _ (TV "struct")) _ rs) = [(v, m, rs)] <> concatMap (seekRecs m . snd) rs
...
structName i (FV v (TV "struct")) = "mlc_" <> pretty v <> "_" <> pretty i
...
generateSourcedSerializers es0 = do
  ...
  where
    ...
    makeSerial _ (NamT _ (TV "struct") _ _, _) = Nothing
    makeSerial _ (NamT r (TV v) _ rs, ps)
