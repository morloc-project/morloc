...
translateSegment m0 =
  ...
  where
    ...
    makeSerialExpr _ (AppPoolS_ _ (PoolCall mid (Socket _ _ socketFile) ForeignCall args) _) = do
      let call =
            [idoc|morloc_foreign_call(#{makeSocketPath socketFile}, #{pretty mid}L, list#{tupled (map argNamer args)})|]
    ...
    makeSerialExpr _ (AppPoolS_ _ (PoolCall mid (Socket _ _ socketFile) (RemoteCall res) args) _) = do
      let resMem = pretty $ remoteResourcesMemory res
          ...
          call =
            "morloc_remote_call"
              <> tupled [pretty mid, dquotes socketFile, dquotes ".morloc-cache", resources, argList]
      return $ defaultValue {poolExpr = call}
    ...
    makeNativeExpr _ (ListN_ v _ xs) = return $ mergePoolDocs rlist xs
      where
        rlist es' = case v of
