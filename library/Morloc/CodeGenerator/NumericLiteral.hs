...
resolveNumericLiteral i src s = case (src, s) of
  (IntSrc n, SerialInt8  _)   -> checkIntRange i "I8"   n (-128) 127                >> pure (ResolvedInt I8   n)
  (IntSrc n, SerialInt16 _)   -> checkIntRange i "I16"  n (-32768) 32767            >> pure (ResolvedInt I16  n)
  (IntSrc n, SerialInt32 _)   -> checkIntRange i "I32"  n (-2147483648) 2147483647  >> pure (ResolvedInt I32  n)
  (IntSrc n, SerialInt64 _)   -> checkIntRange i "I64"  n (-9223372036854775808) 9223372036854775807
                                                                                     >> pure (ResolvedInt I64  n)
  (IntSrc n, SerialInt   _)   -> pure (ResolvedInt IVar n)
  (IntSrc n, SerialUInt8  _)  -> checkIntRange i "U8"  n 0 255                     >> pure (ResolvedInt U8   n)
  (IntSrc n, SerialUInt16 _)  -> checkIntRange i "U16" n 0 65535                   >> pure (ResolvedInt U16  n)
  (IntSrc n, SerialUInt32 _)  -> checkIntRange i "U32" n 0 4294967295              >> pure (ResolvedInt U32  n)
  (IntSrc n, SerialUInt64 _)  -> checkIntRange i "U64" n 0 18446744073709551615    >> pure (ResolvedInt U64  n)
  ...
  (RealSrc r, SerialFloat32 _) -> checkRealRange i "F32" f32Bound r checkF32Overflow >> pure (ResolvedReal F32 r)
  (RealSrc r, SerialFloat64 _) -> checkRealRange i "F64" f64Bound r checkF64Overflow >> pure (ResolvedReal F64 r)
  (RealSrc r, SerialReal    _) -> checkRealRange i "F64" f64Bound r checkF64Overflow >> pure (ResolvedReal F64 r)
...
integerSerialName (SerialInt8   _) = Just "I8"
integerSerialName (SerialInt16  _) = Just "I16"
integerSerialName (SerialInt32  _) = Just "I32"
integerSerialName (SerialInt64  _) = Just "I64"
integerSerialName (SerialInt    _) = Just "Int"
integerSerialName (SerialUInt8  _) = Just "U8"
integerSerialName (SerialUInt16 _) = Just "U16"
integerSerialName (SerialUInt32 _) = Just "U32"
integerSerialName (SerialUInt64 _) = Just "U64"
...
promoteInt i kind bound n =
  let sci = fromInteger n :: DS.Scientific
      (targetName, overflows) = case kind of
        F32 -> ("F32" :: MDoc, checkF32Overflow)
        F64 -> ("F64",         checkF64Overflow)
  in if overflows sci
