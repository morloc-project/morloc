...
-- no arguments) is an opaque @impl Fn(&A..) -> R@, which the C++ member
-- types @std::function<R(A..)>@.
rustReturnType :: TypeM -> RustM MDoc
rustReturnType (Function ts o) = do
  argTs <- mapM (fmap ("&" <>) . rustReturnType) ts
  retT <- rustReturnType o
  return $ "impl Fn" <> tupled argTs <+> "->" <+> retT
...
translate srcs es = do
  ...
  -- A crossing closure whose argument or result is itself a closure (a
  -- function whose result is a suspension, or that takes one) has no
  -- spelling as a Rust type without boxing the inner function value, which
  -- this member does not yet do. Refuse the program rather than emit Rust
  -- that rustc rejects. A closure that stays in the pool may capture or
  -- return a closure freely.
  CM.forM_ es $ \sm@(SerialManifold i _ _ _ _) ->
    CM.when (any nestedClosure (collectSerializedClosures sm)) $
      MM.throwSourcedError i
        "A function value whose argument or result is itself a function (a suspension among them) cannot cross into or out of a Rust pool yet; run the suspension (x <- e) before it crosses, or realize this function in another language."
  ...
  defaultValue
    { surroundSerialManifoldM = \recurse sm@(SerialManifold _ _ form _ body) ->
        let carried = loopCarriedIdsSM sm
         in withManifoldScope (borrowedIndicesOfForm form) (sharedIndicesSM sm `Set.union` carried) carried (rustReturnIsClosure (typeMof body)) (recurse sm)
    , surroundNativeManifoldM = \recurse nm@(NativeManifold _ _ form body) ->
        withManifoldScope (borrowedIndicesOfForm form) (sharedIndicesNM nm) Set.empty (rustReturnIsClosure (typeMof body)) (recurse nm)
    }
...
-- result.
nestedClosure :: SerialAST -> Bool
nestedClosure (SerialClosure ins out) = any isClosureAst (out : ins)
  where
    isClosureAst (SerialClosure _ _) = True
    isClosureAst _ = False
