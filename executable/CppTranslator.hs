...
renderTemplatedType :: Text -> [MDoc] -> MDoc
renderTemplatedType name argDocs
  | T.any (== '$') name = pretty $ expandMacro name (map render argDocs)
    ...
    where
      ...
      f (AppF (VarF (FV gv@(TV gvText) (CV cvText))) ts) | gvText == cvText = do
        ...
        if legit
          then do
            ts' <- mapM f ts
            return . pretty $ expandMacro cvText (map render ts')
      ...
      f (NamF _ (FV _ (CV s)) ps _) = do
        ps' <- mapM f ps
        return $ renderTemplatedType s ps'
      ...
      f (RecF (FV gv@(TV _) _)) = do
        ...
        case Map.lookup gv cscope of
          Just ((_, body, _, _) : _) | Just cv <- bodyCV body -> return (pretty cv)
      ...
      cscopeMatches gv cvText = do
        ...
        return $ case Map.lookup gv cscope of
          Just entries -> any (\(_, body, _, _) -> bodyCV body == Just cvText) entries
          Nothing -> False
...
generateSourcedSerializers univeralScopeMap scopeMap es0 = do
  ...
  where
    ...
    showDefType self ps (NamT _ (TV v) args _) =
      let argDocs = filter (not . MT.null . render) [showDefType self ps a | a <- args]
      in renderTemplatedType v argDocs
    showDefType _ _ (NatLitT _) = mempty
    showDefType _ _ NatVoidT = mempty
    showDefType self ps (AppT (VarT (TV v)) ts) =
      pretty $ expandMacro v (map (render . showDefType self ps) ts)
