...
instance {-# OVERLAPPABLE #-} (HasTypeF e) => HasCppType e where
  cppTypeOf = f . typeFof
    where
      ...
      -- Back-reference to a recursive alias or record. The cycle was
      -- cut by 'recRef' in Infer.hs; the @Maybe CVar@ slot encodes
      -- whether the user supplied a concrete C++ mapping.
      --
      -- @Nothing@: no @type Cpp => ...@ / @record Cpp => ...@ mapping
      -- exists for this alias. Synthesizing a CVar from the morloc-side
      -- TVar text would emit an undeclared identifier into pool.cpp
      -- (the @Pair was not declared in this scope@ leak this
      -- representation prevents at the type level). Raise a meaningful
      -- diagnostic instead.
      f (RecF (TV gv) Nothing) =
        error $
      ...
      -- Mapped case: carries the cscope-resolved concrete name (e.g.
      -- "tree_t" for @record Cpp => Tree = "tree_t"@, "pair_t<$1>" for
      -- @type Cpp => Pair a = "pair_t<$1>" a@). The surrounding context
      -- (vector/optional/pointer) supplies the required indirection so
      -- a bare struct name here is enough.
      f (RecF _ (Just (CV s))) = return $ pretty s
      f (EffectF _ t) = do
        t' <- f t
  ...
  LowerConfig
    ...
    , lcRecordConstructor = \recType _ _ _ rs -> do
        ...
        let v' = "a" <> pretty idx
            ...
            isSelfRef (FV gv _) (RecF rv _) = gv == rv
            isSelfRef _ _ = False
