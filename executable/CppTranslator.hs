...
instance {-# OVERLAPPABLE #-} (HasTypeF e) => HasCppType e where
  cppTypeOf = f . typeFof
    where
      ...
      -- Back-reference to a recursive record. The cycle is cut by RecF
      -- so we render only the struct name; the surrounding context
      -- (vector/optional/pointer) supplies the required indirection.
      --
      ...
      f (RecF (FV _ s)) = return $ pretty s
      f (EffectF _ t) = do
        t' <- f t
        return $ "std::function<" <> t' <> "()" <> ">"
      -- Recursive optional (?T where T points back to a containing record)
      -- gets an explicit unique_ptr indirection. Two pressures meet:
      --   (1) C++'s incomplete-type rule forbids `optional<T>` inside T's
      --       own body -- optional needs `sizeof(T)` which is not yet
      --       available while T is being parsed.
      --   (2) The voidstar wire format for Optional is already a relptr
      --       (the 8-byte slot in the buffer), so the natural C++
      --       encoding of "pointer-or-null" is a pointer-flavored type.
      -- A non-recursive `?T` keeps `std::optional<T>` -- the marshalling
      -- layer in cppmorloc.hpp dereferences the wire-format relptr and
      -- constructs an inline optional.
...
generateSourcedSerializers univeralScopeMap scopeMap es0 = do
  ...
  where
    ...
    -- struct's name. Used to decide whether an `?T` field needs the
    -- `unique_ptr` indirection at C++ emission. Walks through aliasing
    -- shapes (VarT and an already-expanded NamT) but not through
    -- containers -- a `?[T]` field is fine because list-of-incomplete
    -- is allowed and the recursion is broken by the vector indirection.
    ...
    showDefType _ _ (NamT _ v _ _) = pretty v
    ...
    showDefType self ps (OptionalT t)
      | isRecursiveRef self t =
          "std::optional<std::unique_ptr<" <> showDefType self ps t <> ">>"
      | otherwise =
