...
installTest env name spec = testCase name $ do
  ...
  withTestCopy srcDir $ \workDir -> do
    ...
    mapM_ (\f -> assertFileExists (name ++ ": " ++ f ++ " included") (exePath </> f))
          (isFiles spec)

    -- Check expected directories
    mapM_ (\d -> assertDirExists (name ++ ": " ++ d ++ " included") (exePath </> d))
          (isDirs spec)
...
installTests env = testGroup "Install"
  [ testGroup "Python"
    [ installTest env "testpy1" $ InstallSpec
    ...
    , installTest env "testpy2" $ InstallSpec
    ...
    , installTest env "testpy3" $ InstallSpec
  ...
  , testGroup "Cpp"
    [ installTest env "testcpp1" $ InstallSpec
    ...
    , installTest env "testcpp2" $ InstallSpec
    ...
    , installTest env "testcpp3" $ InstallSpec
  ...
  , testGroup "R"
    [ installTest env "testr1" $ InstallSpec
    ...
    , installTest env "testr2" $ InstallSpec
    ...
    , installTest env "testr3" $ InstallSpec
