...
import System.Process (CreateProcess(cwd, std_err, std_out), ProcessHandle,
                       StdStream(..), proc, createProcess,
...
runNexusQuiet workDir subcmd args = do
  ...
  let cp = (proc (workDir </> "nexus") (subcmd : args))
            { cwd = Just workDir
...
withDaemon
  :: FilePath    -- ^ working directory with compiled nexus
  -> [String]    -- ^ extra daemon args (e.g. ["--http-port", "12345"])
  -> (DaemonHandle -> IO a)
  -> IO a
withDaemon workDir extraArgs action = bracket startD stopD action
  where
    startD = do
      devNull <- openFile "/dev/null" WriteMode
      let cp = (proc (workDir </> "nexus") ("--daemon" : extraArgs))
                { cwd = Just workDir
                , std_out = UseHandle devNull
                , std_err = UseHandle devNull
                }
...
pickFreePort :: IO Int
pickFreePort = do
  (_, out, _) <- readProcessWithExitCode "python3"
...
waitForHttp port maxWaitMs = go 0
  where
    ...
    go elapsed
      ...
      | otherwise = do
          result <- try $ httpGet ("http://127.0.0.1:" ++ show port ++ "/health")
...
httpPost url body = readProcessWithExitCode "curl"
...
countZombies :: IO Int
countZombies = do
  (_, out, _) <- readProcessWithExitCode "sh" ["-c", "ps -eo stat 2>/dev/null | grep -c '^Z' || echo 0"] ""
...
assertContains label needle haystack =
  assertBool (label ++ ": expected to contain " ++ show needle ++ " in " ++ show (take 200 haystack))
...
assertJsonEq label rawJson field expected = do
  ...
  if val == expected
    then return ()
    else assertFailure $ label ++ ": field " ++ show field
           ++ " expected " ++ show expected ++ " but got " ++ show val
           ++ "\n  raw response: " ++ show (take 500 rawJson)
