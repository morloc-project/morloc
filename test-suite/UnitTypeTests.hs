...
unitTypeTests =
  testGroup
    ...
    --     (lst num)
    -- , expectError
    --     "applications of non-funcions should fail (1)"
    --     "f = 5; g = \\x -> f x; g 12"
    --     NonFunctionDerive
    -- , expectError
    --     "applications of non-funcions should fail (2)"
    --     "f = 5; g = \\h -> h 5; g f"
    --     NonFunctionDerive
    -- -- binding
    -- , exprTestGood
    ...
    --     ]
    -- , (flip $ expectError "fail on import of Main") CannotImportMain $
    --   T.unlines
    --     ["module Main {export x; x = 42};", "module Foo {import Main (x)}"]
    -- , (flip $ expectError "fail on import of non-existing variable")
    --     (BadImport (MV "Foo") (EV "x")) $
    --   T.unlines
    --     ["module Foo {export y; y = 42};", "module Main {import Foo (x); x}"]
    -- , (flip $ expectError "fail on cyclic dependency") CyclicDependency $
    --   T.unlines
    --     [ "module Foo {import Bar (y); export x; x = 42};"
    --     , "module Bar {import Foo (x); export y; y = 88}"
    --     ]
    -- , (flip $ expectError "fail on redundant module declaration")
    --     (MultipleModuleDeclarations (MV "Foo")) $
    --   T.unlines ["module Foo {x = 42};", "module Foo {x = 88}"]
    -- , (flip $ expectError "fail on self import") (SelfImport (MV "Foo")) $
    --   T.unlines ["module Foo {import Foo (x); x = 42}"]
    -- , (flip $ expectError "fail on import of non-exported variable")
    --     (BadImport (MV "Foo") (EV "x")) $
