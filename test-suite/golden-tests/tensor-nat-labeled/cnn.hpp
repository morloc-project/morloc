...
mlc::Tensor2<double> makeImage(int h, int w) {
    mlc::Tensor2<double> img(std::array<int64_t, 2>{(int64_t)h, (int64_t)w});
    auto iv = img.view();
    int midR = h / 2;
    int midC = w / 2;
    // vertical bar
    for (int i = 0; i < h; i++) iv(i, midC) = 1.0;
    // horizontal bar
    for (int j = 0; j < w; j++) iv(midR, j) = 1.0;
    return img;
}
...
mlc::Tensor3<double> makeKernels(int k, int fh, int fw) {
    mlc::Tensor3<double> kern(std::array<int64_t, 3>{(int64_t)k, (int64_t)fh, (int64_t)fw});
    int ksize = fh * fw;
    if (k >= 1) {
...
// Bias per filter (Vector -> std::vector)
std::vector<double> makeBias(int k) {
    std::vector<double> b(k, 0.1);
    return b;
}
    ...
    const std::vector<double>& bias)
{
    ...
    mlc::Tensor3<double> out(std::array<int64_t, 3>{F, Oh, Ow});
    auto iv = image.view();
...
mlc::Tensor3<double> reluMap(const mlc::Tensor3<double>& t) {
    mlc::Tensor3<double> out(std::array<int64_t, 3>{t.shape()[0], t.shape()[1], t.shape()[2]});
...
// Flatten 3D to 1D (Vector -> std::vector)
std::vector<double> flatten3d(const mlc::Tensor3<double>& t) {
    size_t n = t.size();
...
mlc::Tensor2<double> makeWeights(int out_dim, int in_dim) {
    mlc::Tensor2<double> W(std::array<int64_t, 2>{(int64_t)out_dim, (int64_t)in_dim});
    auto wv = W.view();
    for (int i = 0; i < out_dim; i++) {
        for (int j = 0; j < in_dim; j++) {
            if (i == 0) wv(i, j) = 0.01 * (j % 5 - 2);  // small, centered around 0
            else if (i == 1) wv(i, j) = 0.1;              // uniformly positive
            else wv(i, j) = -0.05;                         // uniformly negative
        }
    }
    return W;
}
...
std::vector<double> makeDenseBias(int n) {
