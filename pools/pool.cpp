...
uint8_t* m1(const uint8_t* s0)
{
    try
    {
        int n0 = _get_value<int>(s0, "<int>i4");
        int n1 = morloc_id(n0);
        return(_put_value(n1, "<int>i4"));
    } catch (const std::exception& e)
      {
          std::string error_message = "Error raised in C++ pool by m1:\n" + std::string(e.what());
