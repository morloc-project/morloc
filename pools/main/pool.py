...
sys.path = [os.path.expanduser("."), os.path.expanduser("/home/z/.local/share/morloc/opt"), os.path.expanduser("/home/z/.local/share/morloc/src/morloc/plane")] + sys.path
import importlib
import pymorloc as morloc
test_suite_golden_tests_infix_generic_foo = importlib.import_module("test-suite.golden-tests.infix-generic.foo")
...
def m72():
    try:
        n1 = test_suite_golden_tests_infix_generic_foo.add(6, 5)
    except Exception as e:
            raise RuntimeError(f"Error (pool daemon in m72):\n{e!s}")
    return(n1)

def m1():
    try:
        n2 = test_suite_golden_tests_infix_generic_foo.morloc_str(m72())
    except Exception as e:
            raise RuntimeError(f"Error (pool daemon in m1):\n{e!s}")
    return(morloc.put_value(n2, "<str>s"))
def m78(n0):
    try:
        n1 = test_suite_golden_tests_infix_generic_foo.add(6, n0)
    except Exception as e:
            raise RuntimeError(f"Error (pool daemon in m78):\n{e!s}")
    return(n1)

def m2():
    try:
        n2 = [1, 2, 3]
        n3 = test_suite_golden_tests_infix_generic_foo.morloc_map( functools.partial(m78)
        , n2 )
    except Exception as e:
            raise RuntimeError(f"Error (pool daemon in m2):\n{e!s}")
    return(morloc.put_value(n3, "<list>a<int>i4"))
def m87():
    try:
        n1 = test_suite_golden_tests_infix_generic_foo.add(6, 7)
    except Exception as e:
            raise RuntimeError(f"Error (pool daemon in m87):\n{e!s}")
    return(n1)

def m3():
    try:
        n2 = test_suite_golden_tests_infix_generic_foo.morloc_str(m87())
    except Exception as e:
            raise RuntimeError(f"Error (pool daemon in m3):\n{e!s}")
    return(morloc.put_value(n2, "<str>s"))
def m106(n1):
    try:
        s2 = m4(morloc.put_value(n1, "<int>i4"))
    except Exception as e:
            raise RuntimeError(f"Error (pool daemon in m106):\n{e!s}")
    return(morloc.get_value(s2, "<int>i4"))

def m4(s1):
    try:
        n1 = morloc.get_value(s1, "<int>i4")
        n3 = test_suite_golden_tests_infix_generic_foo.morloc_str(m106(n1))
    except Exception as e:
            raise RuntimeError(f"Error (pool daemon in m4):\n{e!s}")
    return(morloc.put_value(n3, "<str>s"))
